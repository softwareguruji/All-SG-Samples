	...
	public static void main(String[] args) {
		...
		for (int testCase = 0; testCase < totalTestCase; testCase++) {
			...
			for(int i = 0; i < ticketsPerHouse.length - 2; i++) {
				for(int j=i+2; j<ticketsPerHouse.length; j = j + 2) {
					//System.out.println(i + "," + j + " = " + (ticketsPerHouse[i] + ticketsPerHouse[j]));
					//System.out.println();
					if(maxNumberTicket < (ticketsPerHouse[i] + ticketsPerHouse[j])) {
						maxNumberTicket = (ticketsPerHouse[i] + ticketsPerHouse[j]);
						isCombined = true;
						leftNumber = ticketsPerHouse[i];
						rightNumber = ticketsPerHouse[j];
					} else if(isCombined && maxNumberTicket == (ticketsPerHouse[i] + ticketsPerHouse[j])) {
						if(ticketsPerHouse[j] > rightNumber) {
							leftNumber = ticketsPerHouse[i];
							rightNumber = ticketsPerHouse[j];
						}
					}
